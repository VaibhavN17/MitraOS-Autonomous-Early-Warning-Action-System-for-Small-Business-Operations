...
export const ChatDrawer: React.FC<ChatDrawerProps> = ({
  ...
  // Initialize welcome message when merchant changes
  useEffect(() => {
    if (currentMerchant) {
      setMessages([
        {
          id: 'init',
          sender: 'agent',
          text: `Hello! I'm Mitra, your autonomous operations copilot for ${currentMerchant.name}. I'm continuously monitoring your Razorpay payment streams, inventory velocity, and customer cohorts. How can I assist your operations today?`,
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      const errMsg: ChatMessage = {
        id: Date.now().toString(),
        sender: 'agent',
        text: `I encountered an issue processing telemetry: ${err.message}`,
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      };
    ...
    try {
      ...
      setMessages(prev => [
        ...prev,
        {
          id: Date.now().toString(),
          sender: 'agent',
          text: `✓ Plan "${plan.title}" has been authorized and dispatched via Razorpay APIs. Operations and telemetry have updated.`,
          timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
        }
      ]);
    } catch (err: any) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden animate-in fade-in duration-100">
      ...
      <div className="fixed inset-y-0 right-0 max-w-full flex pl-10">
        <div className="w-screen max-w-lg bg-white shadow-modal flex flex-col border-l border-slate-200 animate-in slide-in-from-right duration-200">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <div className="flex items-center space-x-2">
                  <h3 className="font-bold text-slate-900 text-sm font-display">Mitra AI Copilot</h3>
          ...
          <div className="flex-1 p-5 overflow-y-auto space-y-4 bg-slate-50/50">
            {messages.map((m) => (
              ...
              >
                <div className="flex items-start space-x-2 max-w-[90%]">
                  ...
                  {m.sender === 'user' && (
                    <div className="w-6 h-6 rounded-lg bg-slate-200 text-slate-700 flex items-center justify-center shrink-0 mt-0.5 text-[10px] font-bold">
                      YOU
                    </div>
                  )}
                </div>
                ...
                {m.structured_plan && (
                  <div className="mt-2.5 ml-8 max-w-[90%] saas-card p-4 space-y-2.5">
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-center space-x-1.5 py-2 px-3 bg-slate-900 hover:bg-indigo-600 text-white rounded-lg text-xs font-bold transition-all shadow-xs active:scale-95 disabled:bg-slate-400"
                    >
                      {executingPlanId === m.structured_plan.action_id ? (
                        <>
                          <Loader2 className="w-3.5 h-3.5 animate-spin" />
                          <span>Dispatching via Razorpay...</span>
                        </>
                      ) : (
                        <>
            ...
            {loading && (
              <div className="flex items-center space-x-2 text-xs text-slate-500 ml-8 bg-white p-2.5 rounded-xl border border-slate-200 w-fit shadow-xs">
          ...
          {/* Quick Prompts */}
          <div className="p-2.5 bg-white border-t border-slate-100 flex items-center space-x-2 overflow-x-auto text-[11px] scrollbar-none">
            {quickPrompts.map((prompt, idx) => (
              <button
                key={idx}
                onClick={() => handleSendMessage(prompt)}
                className="whitespace-nowrap px-3 py-1 rounded-lg bg-slate-100 hover:bg-slate-200/80 text-slate-700 font-semibold transition-colors shrink-0"
              >
                {prompt}
              </button>
            ))}
          ...
          <div className="p-3.5 bg-white border-t border-slate-200">
            ...
            >
              <input
                ...
                placeholder={`Ask Mitra about ${currentMerchant?.name} operations...`}
                className="flex-1 text-xs border border-slate-300 rounded-xl px-3.5 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500 font-medium"
              />
              <button
                type="submit"
                disabled={!inputValue.trim() || loading}
                className="bg-slate-900 hover:bg-indigo-600 disabled:bg-slate-300 text-white p-2.5 rounded-xl transition-colors shadow-xs shrink-0"
              >
